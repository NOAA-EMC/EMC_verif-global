...
# Author(s)/Contact(s): Ho-Chun Huang
# Abstract: Run Python code for global satellite verification
#           to create plots from step 1
# History Log:
#   9/2020: Initial version of script
#   1/2026: Revised for new MET and METPlus version in step1
#
# Usage:
#   Parameters:
#       agrument to script
...
# Create output directories for plot
...
for group in condense_stats filter_stats make_plots; do
    ...
    python $USHverif_global/plots/satellite/step2_satellite_create_job_scripts.py
    status=$?
    [[ $status -ne 0 ]] && exit $status
    [[ $status -eq 0 ]] && echo "Successfully ran step2_satellite_create_job_scripts.py"
...
# Tar up plots
python $USHverif_global/plots/satellite/step2_satellite_tar_images.py
status=$?
[[ $status -ne 0 ]] && exit $status
[[ $status -eq 0 ]] && echo "Successfully ran step2_satellite_tar_images.py"
