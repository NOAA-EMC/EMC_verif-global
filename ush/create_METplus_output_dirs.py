...
if RUN == 'grid2grid_step1':
    ...
    for type in os.environ['g2g1_type_list'].split(' '):
       for model in model_list:
           metplus_output_subdir_list.append('make_met_data_by_'+make_met_data_by+'/grid_stat/'+type+'/'+model)
           metplus_output_subdir_list.append('gather_by_'+gather_by+'/stat_analysis/'+type+'/'+model) 
elif RUN == 'grid2obs_step1':
    gather_by = os.environ['g2o1_gather_by']
    for type in os.environ['g2o1_type_list'].split(' '):
        metplus_output_subdir_list.append('make_met_data_by_'+make_met_data_by+'/pb2nc/'+type+'/prepbufr')
        for model in model_list:
            metplus_output_subdir_list.append('make_met_data_by_'+make_met_data_by+'/point_stat/'+type+'/'+model)
            metplus_output_subdir_list.append('gather_by_'+gather_by+'/stat_analysis/'+type+'/'+model)
elif RUN == 'precip_step1':
    gather_by = os.environ['precip1_gather_by'] 
    for type in os.environ['precip1_type_list'].split(' '):
        for model in model_list:
            metplus_output_subdir_list.append('make_met_data_by_'+make_met_data_by+'/pcp_combine/'+type+'/'+model)
            metplus_output_subdir_list.append('make_met_data_by_'+make_met_data_by+'/grid_stat/'+type+'/'+model)
