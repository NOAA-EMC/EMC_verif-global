...
Abstract: This script generates a date by level plot.
          (x-axis: dates; y-axis: pressure levels; contours: statistics values)
          (Graphics Naming Convention: vertprof_date)
'''
...
class DateByLevel:
    ...
    def make_date_by_level(self):
        """! Make the date by level graphic
        
             Args:

             Returns:
        """
        self.logger.info(f"Plot Type: Date By Level...")
        ...
        for level in vert_profile_levels:
            ...
            level_stat_df_dict[level] = stat_df
        level_stat_df = pd.concat(level_stat_df_dict,
                                  names=['level', 'model', 'valid_dates'])
        # Set up plot
        ...
        if nsubplots == 1:
            ...
            sys.exit(1)
        if nsubplots <= 2:
            plot_specs_dbl.fig_size = (16., 8.)
            plot_specs_dbl.fig_title_size = 16
            plt.rcParams['figure.titlesize'] = plot_specs_dbl.fig_title_size
        if nsubplots >= 2:
            n_xticks = 2
        else:
            n_xticks = 3
        ...
        if len(fcst_units) > 1:
            ...
            fcst_units = ['']
        plot_title = plot_specs_dbl.get_plot_title(
        ...
        if os.path.exists(plot_left_logo_path):
            ...
            left_logo_xpixel_loc, left_logo_ypixel_loc, left_logo_alpha = (
                plot_specs_dbl.get_logo_location(
                    'left', plot_specs_dbl.fig_size[0],
                    plot_specs_dbl.fig_size[1], plt.rcParams['figure.dpi']
                )
            )
        else:
        ...
        if os.path.exists(plot_right_logo_path):
            ...
            right_logo_xpixel_loc, right_logo_ypixel_loc, right_logo_alpha = (
                plot_specs_dbl.get_logo_location(
                    'right', plot_specs_dbl.fig_size[0],
                    plot_specs_dbl.fig_size[1], plt.rcParams['figure.dpi']
                )
            )
        else:
        ...
        image_name = plot_specs_dbl.get_savefig_name(
            self.output_dir, self.plot_info_dict, self.date_info_dict
        )
        subplot0_cmap, subplotsN_cmap = plot_specs_dbl.get_plot_colormaps(
            self.plot_info_dict['stat']
        )
        subplot0_data = [
            i.to_numpy().tolist() for _,i in \
            level_stat_df.loc[:,model_idx_list[0],:].groupby('valid_dates')
        ]
        for model_idx in model_idx_list[1:]:
            subplotN_data = [
                i.to_numpy().tolist() for _,i in \
                level_stat_df.loc[:,model_idx,:].groupby('valid_dates')
            ]
        ...
        (have_subplot0_levs, subplot0_levs,
         have_subplotsN_levs, subplotsN_levs) = (
            plot_specs_dbl.get_plot_contour_levels(
        ...
        fig = plt.figure(figsize=(plot_specs_dbl.fig_size[0],
                                  plot_specs_dbl.fig_size[1]))
                ...
                if not subplot0_data.mask.all():
                    ...
                    if self.plot_info_dict['stat'] in ['BIAS', 'ME', 'FBIAS']:
                        if not make_colorbar:
                            ...
                            cbar_label = plot_specs_dbl.get_stat_plot_name(
                                self.plot_info_dict['stat']
                            )
                else:
                    ...
                    if have_subplotsN_levs:
                        ...
                        if not make_colorbar:
                            ...
                            if self.plot_info_dict['stat'] in ['BIAS', 'ME',
                                                               'FBIAS']:
                                cbar_label = (
                                    plot_specs_dbl.get_stat_plot_name(
                                        self.plot_info_dict['stat']
                                    )
                                )
                            else:
...
def main():
    ...
    p = DateByLevel(logger, INPUT_DIR, OUTPUT_DIR, MODEL_INFO_DICT,
                    DATE_INFO_DICT, PLOT_INFO_DICT, MET_INFO_DICT, LOGO_DIR)
    p.make_date_by_level()
