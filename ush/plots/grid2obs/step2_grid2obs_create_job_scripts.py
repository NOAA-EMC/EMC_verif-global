'''
Program Name: step2_grid2obs_create_job_scripts.py
Contact(s): Shannon Shields
Abstract: This script is run by exgrid2obs_step2.sh in scripts/.
...
base_plot_jobs_info_dict = {
    'grid2obs_upper_air': {
        'TMP': {'vx_masks': ['G003','NH', 'SH', 'TRO', 'G236', 'POLAR','ARCTIC'],
    ...
    'grid2obs_conus_sfc': {
        'TMP2m': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                   ...
                   'obs_var_dict': {'name': 'TMP',
                                    'levels': ['Z2']},
                   'obs_name': 'ADPSFC'},            
        'RH2m': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                               'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                               'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'], 
                     'fcst_var_dict': {'name': 'RH',
                                       'levels': ['Z2']},
                     'obs_var_dict': {'name': 'RH',
                                      'levels': ['Z2']},
                     'obs_name': 'ADPSFC'},
        'DPT2m': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                               'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                               'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'], 
                       ...
                       'obs_var_dict': {'name': 'DPT',
                                        'levels': ['Z2']},
                       'obs_name': 'ADPSFC'},            
        'TCDC': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                               'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                               'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'],
        ...
        'PRMSL': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                         ...
                         'obs_var_dict': {'name': 'PRMSL',
                                          'levels': ['Z0']},
                         'obs_name': 'ADPSFC'},
        'VISsfc': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                                    'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                                    'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'],
                       ...
                       'obs_var_dict': {'name': 'VIS',
                                        'levels': ['Z0']},
                       'obs_name': 'ADPSFC'},        
        'CAPEsfc': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                                    'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                                    'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'],
                         ...
                         'obs_var_dict': {'name': 'CAPE',
                                          'levels': ['L100000-0']}, 
                         'obs_name': 'ADPUPA'},
        'HGTcldceil': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                                    'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                                    'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'],
                            'fcst_var_dict': {'name': 'HGT',
                                      'levels': ['L0']},
                             'obs_var_dict': {'name': 'HGT',
                                     'levels': ['L0']},
                             'obs_name': 'ADPSFC'},
        'HPBL': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                              'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                              'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'],
                      ...
                      'obs_var_dict': {'name': 'HPBL',
                                       'levels': ['L0']},
                      'obs_name': 'ADPUPA'},
        'GUSTsfc': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                                 'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                                 'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'],
                     ...
                     'obs_var_dict': {'name': 'GUST',
                                      'levels': ['Z0']},
                     'obs_name': 'ADPSFC'},        
        'UGRD_VGRD10m': {'vx_masks': ['G104','WEST', 'EAST', 'MDW', 'NPL', 'SPL', 'NEC',
                                      'SEC', 'NWC', 'SWC', 'NMT', 'SMT', 'SWD',
                                      'GRB', 'LMV', 'GMC', 'APL', 'NAK', 'SAK'],
                     'fcst_var_dict': {'name': 'UGRD_VGRD',
                                       'levels': ['Z10']},
...
#### pres_levs
for pres_levs_job in list(condense_stats_jobs_dict['grid2obs_upper_air'].keys()):
...
# upper_air
pres_levs_stats = os.environ["g2o2_upper_air_stats_list"]
...
for pres_levs_job in list(make_plots_jobs_dict['grid2obs_upper_air'].keys()):
    ...
    make_plots_jobs_dict['grid2obs_upper_air'][pres_levs_job]['line_type_stats'] = [
        ...
        for s in pres_levs_stats.split(' ')
    ]
    ...
    if pres_levs_job_line_type == "SL1L2" and \
            and "fbar_obar" in os.environ["g2o2_upper_air_stats_list"]:
...
#### polar_sfc
psfc_stats = os.environ["g2o2_polar_sfc_stats_list"]
if "fbar_obar" in psfc_stats:
    psfc_stats = psfc_stats.remove("fbar_obar")
for psfc_job in list(make_plots_jobs_dict['grid2obs_polar_sfc'].keys()):
    ...
    make_plots_jobs_dict['grid2obs_polar_sfc'][psfc_job]['line_type_stats'] = [
        f"SL1L2/{s.upper()}" \
        for s in psfc_stats.split(' ')
    ]
    make_plots_jobs_dict['grid2obs_polar_sfc'][psfc_job]['plots'] = [
        'time_series','lead_average'
    ]
    if "fbar_obar" in os.environ["g2o2_conus_sfc_stats_list"]:
    ...
    for case_type_job in list(case_type_plot_jobs_dict.keys()):
        ...
        if JOB_GROUP in ['filter_stats', 'make_plots']:
            ...
            valid_hrs = list(range(valid_hr_start,
                                   ...
                                   valid_hr_inc))
        obs_list = (
            os.environ[RUN_abbrev_type+'_msg_type_list']\
            .split(' ')
        )
        ...
        for loop_info in JOB_GROUP_case_type_job_product_loops:
            if JOB_GROUP in ['condense_stats', 'filter_stats']:
                ...
                for plot_loop_info in list(
                    ...
                    run_verif_global_g2o_plots = ['plots']
                    for run_verif_global_g2o_plot in run_verif_global_g2o_plots:
                        # Set up output directories
                        njobs+=1
                        job_env_dict['job_id'] = 'job'+str(njobs)
                        job_DATA_dir = os.path.join(DATA, RUN, 'plot_output',
