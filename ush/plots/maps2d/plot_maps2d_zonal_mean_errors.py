...
for var_levels_type in list(var_levels_type_dict.keys()):
    ...
    for env_var_model in env_var_model_list:
        ...
        if RUN_type == 'model2obs' and model_num == 1:
            ...
            ax_obs_subplot_loc = (
                str(ax_obs.get_subplotspec().rowspan.start)+','
                +str(ax_obs.get_subplotspec().colspan.start)
            )
            ...
            if get_levels:
                if var_name in ['UGRD', 'VGRD', 'VVEL', 'LFTX',
                    ...
                    levels = maps2d_plot_util.get_clevels(ax_obs_plot_data, 1.25)
                else:
                    levels = np.nan
                get_levels = False
        ...
        if RUN_type == 'model2model' and forecast_anl_diff == 'YES':
            ...
            ax_anl_subplot_loc = (
                str(ax_anl.get_subplotspec().rowspan.start)+','
            ...
            if get_diff_levels:
                levels_diff = maps2d_plot_util.get_clevels(ax_anl_plot_data, 1.25)
        ...
        if RUN_type == 'model2obs':
            ...
            if get_diff_levels:
                levels_diff = maps2d_plot_util.get_clevels(ax_plot_data, 1.25)
                get_diff_levels = False
            ...
            if model_num == 1:
                ...
                if get_levels:
                    if var_name in ['UGRD', 'VGRD', 'VVEL', 'LFTX',
                                    '4LFTX', 'UFLX', 'VFLX', 'GFLX']:
                        levels = maps2d_plot_util.get_clevels(ax_plot_data, 1.25)
                    else:
                        levels = np.nan
                get_levels = False
