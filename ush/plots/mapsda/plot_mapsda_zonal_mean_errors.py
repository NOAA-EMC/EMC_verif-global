...
import re
import mapsda_plot_util as mapsda_plot_util
import warnings
import logging
import datetime
import matplotlib
...
noaa_logo_img_array = matplotlib.image.imread(
    os.path.join(os.environ['USHverif_global'], 'plots', 'logos', 'noaa.png')
)
noaa_logo_alpha = 0.5
nws_logo_img_array = matplotlib.image.imread(
    os.path.join(os.environ['USHverif_global'], 'plots', 'logos', 'nws.png')
)
nws_logo_alpha = 0.5
...
def plot_subplot_data(ax_tmp, plot_data, plot_data_lat, plot_data_levels,
    ...
    if np.count_nonzero(~np.isnan(plot_data)) != 0:
        ...
        if ax_tmp.get_subplotspec().rowspan.start == 0 \
                and ax_tmp.get_subplotspec().colspan.start == 0:
...
for env_var_model in env_var_model_list:
    ...
    for var_level in var_levels:
        ...
        var_info_title, levels, levels_diff, cmap, var_scale, cbar00_title = (
            mapsda_plot_util.get_maps2d_plot_settings(var_name, var_level)
        )
        if RUN_type == 'gdas':
    ...
    for var_levels_type in list(var_levels_type_dict.keys()):
        ...
        for env_var_model in env_var_model_list:
            ...
            if RUN_type == 'gdas' and model_num == 1:
                ...
                ax_cntrl_subplot_loc = (
                    str(ax_cntrl.get_subplotspec().rowspan.start)+','
                ...
                if get_levels:
                    if var_name in ['UGRD', 'VGRD', 'VVEL', 'LFTX',
                        ...
                        levels = mapsda_plot_util.get_clevels(
            ...
            if stat == 'inc':
                ...
                if model_num == 1:
                    levels_plot = mapsda_plot_util.get_clevels(stat_data, 1.25)
                ...
                if model_num == 1:
                    ...
                    if stat == 'mean':
                        ...
                        if get_levels:
                            if var_name in ['UGRD', 'VGRD', 'VVEL', 'LFTX',
                                            '4LFTX', 'UFLX', 'VFLX', 'GFLX']:
                                levels_plot = mapsda_plot_util.get_clevels(
                                    stat_data, 1.25
                                )
                            else:
                                levels_plot = np.nan
            ...
            ax_subplot_loc = (
                str(ax.get_subplotspec().rowspan.start)+','
                +str(ax.get_subplotspec().colspan.start)
            )
