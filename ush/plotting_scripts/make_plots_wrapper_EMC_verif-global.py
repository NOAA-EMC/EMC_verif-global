...
class MakePlotsWrapper(CommandBuilder):
    ...
    def create_plots_precip(self, fcst_var_level_list, obs_var_level_list,
        ...
        if os.environ['CI_METHOD'] == 'EMC_MONTE_CARLO':
            ...
            randx_dir = os.path.join(os.environ['PLOTTING_OUT_DIR_FULL'], "data",
        ...
        for lead in lead_list:
            ...
            for vl in range(len(fcst_var_level_list)):
                self.add_env_var('FCST_VAR_LEVEL', fcst_var_level_list[vl])
                self.add_env_var('OBS_VAR_LEVEL',obs_var_level_list[vl])
                self.set_plotting_script(os.path.join(plotting_scripts_dir, "plot_threshold_mean.py"))
                self.logger.debug("Running "+os.path.join(plotting_scripts_dir, "plot_threshold_mean.py")+" with...")
                self.logger.debug("DATES: "+os.environ['PLOT_TIME']+" "+os.environ['START_DATE_YYYYmmdd']+" "+os.environ['END_DATE_YYYYmmdd'])
                self.logger.debug("VALID TIME INFO: "+os.environ['VALID_TIME_INFO'])
                self.logger.debug("INIT TIME INFO: "+os.environ['INIT_TIME_INFO'])
                self.logger.debug("FCST VAR: "+os.environ['FCST_VAR_NAME']+" "+fcst_var_level_list[vl]+" "+os.environ["FCST_VAR_THRESH_LIST"]+" "+os.environ['FCST_VAR_EXTRA'])
                self.logger.debug("OBS VAR: "+os.environ['OBS_VAR_NAME']+" "+obs_var_level_list[vl]+" "+os.environ["OBS_VAR_THRESH_LIST"]+" "+os.environ['OBS_VAR_EXTRA'])
                self.logger.debug("INTERP: "+os.environ['INTERP'])
                self.logger.debug("REGION: "+os.environ["REGION"])
        ...
        for vl in range(len(fcst_var_level_list)):
            self.add_env_var('FCST_VAR_LEVEL', fcst_var_level_list[vl])
            self.add_env_var('OBS_VAR_LEVEL',obs_var_level_list[vl])
            self.set_plotting_script(os.path.join(plotting_scripts_dir, "plot_threshold_by_lead.py"))
            self.logger.debug("Running "+os.path.join(plotting_scripts_dir, "plot_threshold_by_lead.py")+" with...")
            self.logger.debug("DATES: "+os.environ['PLOT_TIME']+" "+os.environ['START_DATE_YYYYmmdd']+" "+os.environ['END_DATE_YYYYmmdd'])
            self.logger.debug("VALID TIME INFO: "+os.environ['VALID_TIME_INFO'])
            self.logger.debug("INIT TIME INFO: "+os.environ['INIT_TIME_INFO'])
            self.logger.debug("FCST VAR: "+os.environ['FCST_VAR_NAME']+" "+fcst_var_level_list[vl]+" "+os.environ["FCST_VAR_THRESH_LIST"]+" "+os.environ['FCST_VAR_EXTRA'])
                    ...
                    for region in region_list:
                        ...
                        if verif_case == "precip":
                            self.create_plots_precip(fcst_var_level_list, obs_var_level_list,
                                                     fcst_var_thresh_list, obs_var_thresh_list,
                                                     lead_list, plotting_scripts_dir)
                        elif verif_case == "grid2obs" and verif_type in "polar_sfc":
    ...
    def run_all_times(self):
        ...
        if verif_case == "grid2grid":
            ...
            if verif_type in ("upper_air", "conus_sfc", "polar_sfc"):
                run_make_plots = True
            else:
                run_make_plots = False
