...
for model in model_info:
    ...
    for l in range(len(lead_list)):
        ...
        model_lead_data_now_index = pd.MultiIndex.from_product([[model_plot_name], [lead], expected_stat_file_dates], names=['model_plot_name', 'leads', 'dates'])
        ...
        if os.path.exists(model_stat_file):
            ...
            if nrow == 0:
                ...
                model_lead_now_data = pd.DataFrame(np.nan, index=model_lead_data_now_index, columns=[ 'TOTAL' ])
            else:
                ...
                model_lead_now_data = pd.DataFrame(np.nan, index=model_lead_data_now_index, columns=stat_file_line_type_columns)
...
for stat in plot_stats_list:
    ...
    fig.suptitle(stat_plot_name+"\n"+"Fcst: "+fcst_var_name+"_"+fcst_var_level+fcst_var_extra_title+fcst_var_thresh_title+" Obs: "+obs_var_name+"_"+obs_var_level+obs_var_extra_title+obs_var_thresh_title+" "+interp+" "+grid+"-"+region+"\n"+plot_time+": "+str(datetime.date.fromordinal(int(plot_time_dates[0])).strftime('%d%b%Y'))+"-"+str(datetime.date.fromordinal(int(plot_time_dates[0])).strftime('%d%b%Y'))+", valid: "+valid_time_info[0][0:4]+"-"+valid_time_info[-1][0:4]+"Z, init: "+init_time_info[0][0:4]+"-"+init_time_info[-1][0:4]+"Z\n", fontsize=14, fontweight='bold') 
    savefig_name = os.path.join(plotting_out_dir_imgs, stat+"_timemap_fcst"+fcst_var_name+fcst_var_level+fcst_var_extra+fcst_var_thresh+"_obs"+obs_var_name+obs_var_level+obs_var_extra+obs_var_thresh+"_"+interp+"_"+grid+region+".png")
