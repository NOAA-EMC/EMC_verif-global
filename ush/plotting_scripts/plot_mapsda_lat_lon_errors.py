        ...
        for env_var_model in env_var_model_list:
            ...
            if model_num == 1:
                ...
                if nsubplots == 1:
                    ...
                    gs.update(wspace=0.2, hspace=0.2)
                # Set up control analysis subplot map and title
                if py_map_pckg == 'cartopy':
                ...
                if verif_case_type == 'gdas':
                    ...
                    if stat == 'bias':
                        ...
                        if model_num == 1:
                            ...
                            stat_data_cyc = np.sqrt(
                                (model_data_series_cnt_OBAR_cyc
                                 -model_data_series_cnt_FBAR_cyc)**2
                            )
                            model1_stat_data = stat_data
                            model1_stat_data_cyc = stat_data_cyc
                        else:
                            ...
                            stat_data_cyc = np.sqrt(
                                (model_data_series_cnt_OBAR_cyc
                                 -model_data_series_cnt_FBAR_cyc)**2
                            ) - model1_stat_data_cyc
                    if model_num == 1:
                        levels_diff = plot_util.get_clevels(stat_data)
                    stat_data_area_avg = (
                        maps2d_plot_util.calculate_area_average(
