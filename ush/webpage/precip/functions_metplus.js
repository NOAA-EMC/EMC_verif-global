...
function populateMenu(mode){
	if(mode == 'valid'){
		...
		for(i=0; i<types.length; i++){
			var option = document.createElement("option");
                        option.text = types[i].displayName;
...
function getURL(domain,variable,frame,level,season,validtime,type){
	...
	newurl = newurl.replace("PPP",type);
...
//        document.getElementById("season").selectedIndex = searchByName(imageObj.season,seasons);
	...
	document.getElementById("type").selectedIndex = searchByName(imageObj.type,types);
...
function preload(obj){
	...
	for (i2=0; i2<hrs_loop.length; i2++){
		...
		var urls = getURL(obj.domain,obj.variable,i,obj.level,obj.season,obj.validtime,obj.type);
...
function changeType(id){
	...
	// Domains (Forecast Hours)
	var selected_domain = document.getElementById("domain").value;
        var element = document.getElementById("domain");
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        if( (selected_type=="timeseries") || (selected_type=="threshmean") ){
                type_domains = timeseries_domains;
        }
	else{
                type_domains = fhrmean_domains;
        }
	domains = [];
	for(i=0; i<type_domains.length; i++){
        if( (selected_type=="timeseries") || (selected_type=="threshmean") ){
            domains.push({
                displayName: "F"+type_domains[i],
                name: "_f"+type_domains[i],
            })
        }
	else{
            domains.push({
                displayName: type_domains[i],
                name: "",
            })
        }
        }
	...
	var domains_values= [];
        for(i=0; i<element.options.length; i++){
               domains_values.push(element.options[i].value);
        }
	if(domains_values.indexOf(selected_domain) != -1){
               var idx = domains_values.indexOf(selected_domain);
	...
	var selected_level = document.getElementById("level").value;
        var element = document.getElementById("level");
	for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
	if( (selected_type=="timeseries") || (selected_type=="fhrmean") ){
                type_levels = timeseries_levels;
        }
        else{
                type_levels = threshmean_levels;
        }
	levels = [];
	for(i=0; i<type_levels.length; i++){
	let formattedValue = type_levels[i].toString().replace(/\./g, "p");
        if( (selected_type=="timeseries") || (selected_type=="fhrmean") ){
                 levels.push({
			 displayName: type_levels[i]+" mm",
                         name: "_ge"+formattedValue,
                 })
        }
        else{
                 levels.push({
                         displayName: type_levels[i],
                         name: "",
                 })
        }
	}
	...
	var levels_values= [];
        ...
        if(levels_values.indexOf(selected_level) != -1){
               var idx = seasons_values.indexOf(selected_level);
               element.options[idx].selected = true;
               element.onchange();
        }
        else{
