...
function changeType(id){
	...
	var selected_type = document.getElementById("type").value;
        ...
        //Seasons (Forecast Hours)
        var selected_season = document.getElementById("season").value;
        var element = document.getElementById("season");
        for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        if( (selected_type=="timeseries") || (selected_type=="vertprof") ){
                type_seasons = timeseries_seasons;
        }
        else if( (selected_type=="fhrmean") || (selected_type=="vertprof_fhrmean") ){
                type_seasons = fhrmean_seasons;
        }
        seasons = [];
        for(i=0; i<type_seasons.length; i++){
        if( (selected_type=="timeseries") || (selected_type=="vertprof") ){
            seasons.push({
                displayName: "F"+type_seasons[i],
                name: "_f"+type_seasons[i],
            })
        }
        else if( (selected_type=="fhrmean") || (selected_type=="vertprof_fhrmean") ){
            seasons.push({
                displayName: type_seasons[i],
                name: "",
            })
        }
        }
        ...
        for(i=0; i<element.options.length; i++){
               seasons_values.push(element.options[i].value);
        }
        if(seasons_values.indexOf(selected_season) != -1){
               var idx = seasons_values.indexOf(selected_season);
	...
	if( (selected_type=="timeseries") || (selected_type=="fhrmean") ){
                type_levels = non_vertprof_levels;
		type_levels_name = non_vertprof_levels_name;
        }
        else if( (selected_type=="vertprof") || (selected_type=="vertprof_fhrmean") ){
                type_levels = vertprof_levels;
		type_levels_name = vertprof_levels_name;
        }
	levels = [];
        for(i=0; i<type_levels.length; i++){
        if( (selected_type=="timeseries") || (selected_type=="fhrmean") ){
            levels.push({
                displayName: type_levels_name[i],
                name: type_levels[i],
            })
        }
        else if( (selected_type=="vertprof") || (selected_type=="vertprof_fhrmean") ){
